...
export function DateRangePicker({
  ...
  const [yearPage, setYearPage] = useState((value?.start || new Date()).getFullYear());
  ...
  if (value !== prevValue) {
    ...
    if (value?.start) {
      ...
      setYearPage(value.start.getFullYear());
    }
  }
  ...
  const handleSelectDate = (day: Date) => {
    if (!value.start || (value.start && value.end)) {
      ...
      if (isBefore(day, value.start)) {
        ...
        setIsOpen(false); // Close automatically after selecting end date
      }
    }
  };

  const displayValue = value.start
    ? `${format(value.start, dateFormat)} - ${value.end ? format(value.end, dateFormat) : '...'}`
    : placeholder;
              ...
              <div className="grid grid-cols-7 gap-y-1" onMouseLeave={() => setHoverDate(null)}>
                {days.map((day) => {
                  ...
                  if (value.start && value.end) {
                    inRange = isAfter(day, value.start) && isBefore(day, value.end);
                  } else if (value.start && !value.end && hoverDate) {
                    if (isAfter(hoverDate, value.start)) {
                      inRange =
                        (isAfter(day, value.start) && isBefore(day, hoverDate)) ||
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        isStart &&
                          (value.end || (hoverDate && isAfter(hoverDate, value.start as Date))) &&
                          'bg-gradient-to-r from-transparent to-primary/10',
                        isEnd && 'bg-gradient-to-l from-transparent to-primary/10',
                      )}
                    >
                      <button
                        type="button"
                        ...
                        className={cn(
                          'mx-auto flex h-8 w-8 items-center justify-center rounded-lg text-[13px] font-medium transition-all hover:scale-110',
                          ...
                          inRange && !isStart && !isEnd && 'text-slate-900',
                          (isStart || isEnd) &&
                            'bg-primary font-bold text-white shadow-md hover:bg-primary-hover hover:scale-100 z-10 relative',
                          isTodayDate &&
                            !isStart &&
                            !isEnd &&
                            !inRange &&
